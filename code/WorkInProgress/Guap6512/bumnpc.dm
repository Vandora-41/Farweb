...
/mob/living/carbon/human/bumbot/proc/process()
	...
	if(target)
		...
		if(prob(5))//This percentage used to be way higher, which meant that bums would just like... walk away from you if you weren't right next to them. It was really annoying.
			target = null
		...
		if(target in orange(viewrange,src))
			...
			return 1
	if(prob(20))
		var/obj/machinery/door/airlock/A = locate() in get_step_rand(src)
